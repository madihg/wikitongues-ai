...
async function main() {
  ...
  try {
    ...
    const blocks = [
      { where: "IGALA_SYSTEM_V4_2", text: IGALA_SYSTEM_V4_2 },
      ...
      ...IGALA_SYSTEM_V4_2.split("\n")
        .map((line, i) => ({
          where: `v4.2 line ${i + 1}: ${line.slice(0, 48)}`,
          text: line,
        }))
    ...
    if (report.pass) {
      console.log(
        "SCOPE A: PASS - no frozen gold answer appears in the v4.2 prompt or the v3/v4/v4.1 controls.",
      );
    } else {
